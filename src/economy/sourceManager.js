...
export class sourceManager {
    ...
    addSource(source){
        ...
        for(let i = 0; i < miningSpots.length; i++){
            if(miningSpots[i] != null && !_.isEmpty(miningSpots[i])){
                ...
                const generatedPath = this.createPathToMiningSpot(from, to, miningSpots, miningSpots[i]);
  ...
  createPathToMiningSpot(fromPos, toPos, miningSpots, currentSpot) {
    ...
    let ret = PathFinder.search(fromPos, toPos, {
      ...
      roomCallback(roomName) {
        ...
        for(let spot in miningSpots){
          const spotPos = miningSpots[spot];
